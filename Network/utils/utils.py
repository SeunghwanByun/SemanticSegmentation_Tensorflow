...
def gen_test_output(sess, logits, keep_prob, image_pl, data_folder, image_shape):
    """
        ...
        """
    for i in range(len(glob(os.path.join(data_folder, 'merge', '*.png')))):
        start = time.time()
        image_file_by_train = glob(os.path.join(data_folder, 'merge', '*.png'))[i]
        ...
        im_softmax = sess.run([tf.nn.softmax(logits)], {keep_prob: 1.0, image_pl: [image_by_train]})
